...
data_endpoint=/tmp/pbe-persistent-pd-data-$$.sock
vision_endpoint=/tmp/pbe-persistent-pd-vision-$$.sock
mkdir -p "$run_dir"
...
PYTHONPATH=python "$python_bin" tools/bench/data_flow/run_persistent_pd_deployment_ab.py --vision-endpoint "$vision_endpoint" --language-binary "$build/demo/pbe_vlm_language_role" --model-bin "$model_bin" --tokenizer "$tokenizer" --data-endpoint "$data_endpoint" --image "$image" --device "$device" --output "$run_dir/results.json" | tee "$run_dir/benchmark.log"
"$service" ipc-stats "$data_endpoint" | tee "$run_dir/ipc-after.log"
"$service" stats "$data_endpoint" | tee "$run_dir/data-after.log"
grep -q 'total_slots=128 free_slots=128 active_grants=0' "$run_dir/ipc-after.log"
grep -q 'leases=0' "$run_dir/data-after.log"
"$python_bin" tools/bench/data_flow/validate_persistent_pd_deployment.py --run-dir "$run_dir" --output "$run_dir/validation.json"
