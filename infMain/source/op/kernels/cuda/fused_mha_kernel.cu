// Updated on March 15, 2026
#include <base/cuda_config.h>
#include <tensor/tensor.h>
#include <cfloat>
#include <cub/cub.cuh>
#include "fused_mha_kernel.cuh"
#include <base/tick.h>
namespace kernel {
constexpr static int thread_num = 256;
__global__ void multi_head_attention_kernel(int32_t pos, int32_t seq_len, float* query, 
                                            float* output, float* key_cache, float* value_cache,
                                            int32_t kv_dim, int32_t kv_mul, int32_t head_num,
                                            int32_t head_size, int32_t layer_offset) {
  if (pos >= seq_len) {
    return;
  }

  int32_t head = blockIdx.x;
  if (head >= head_num) {
    return;
  }
  extern __shared__ float s_query_head[];
  float* s_output_head = head_size + s_query_head;
  using BlockReduce = cub::BlockReduce<float, thread_num>;
  __shared__ BlockReduce::TempStorage temp;
  __shared__ float shared_score;
  __shared__ float m;
  __shared__ float m_new;
  __shared__ float alpha;
  __shared__ float p;
  __shared__ float sum;
  for (size_t i = threadIdx.x; i < head_size; i += blockDim.x) {
    s_output_head[i] = 0.f;
  }
  
  float scale = 1.f / sqrtf(float(head_size));
  float* query_head = query + head * head_size;
  for (size_t i = threadIdx.x; i < head_size; i += blockDim.x) {
    s_query_head[i] = query_head[i];
  }
  if (threadIdx.x == 0) {
    m = -FLT_MAX;
    sum = 0.f;
  }
  __syncthreads();
  
  //max_val = BlockReduce(temp).Reduce(max_val, cub::Max());

  for (int32_t t = 0; t < pos + 1; ++t) {
    float* key_pos = key_cache + layer_offset + t * kv_dim + (head / kv_mul) * head_size;
    float score = 0.f;
    for (size_t i = 4 * threadIdx.x; i < head_size; i += 4 * blockDim.x) {
        float4 key_val = *reinterpret_cast<float4*>(key_pos + i);
        float4 query_val = *reinterpret_cast<float4*>(s_query_head +i);
        score += key_val.x * query_val.x + key_val.y * query_val.y + key_val.z * query_val.z + key_val.w * query_val.w;
    }
    score *= scale;
    score = BlockReduce(temp).Sum(score);
    if (threadIdx.x == 0) {
        shared_score = score;
        m_new = max(shared_score, m);
        alpha = expf(m - m_new);
        p = expf(shared_score - m_new);
        sum = sum * alpha + p;
        m = m_new;
    }
    __syncthreads();
    float* value_pos = value_cache + layer_offset + t * kv_dim + (head / kv_mul) * head_size;
    
    for (size_t i = threadIdx.x; i < head_size; i += blockDim.x) {
        s_output_head[i] = value_pos[i] * p + s_output_head[i]* alpha;
    }
    __syncthreads();
  }
  for (size_t i = threadIdx.x; i < head_size; i += blockDim.x) {
    output[head * head_size + i] = s_output_head[i] / sum;
  }
}



void fused_mha_kernel_cu(int32_t pos, int32_t head_num, int32_t layer_index, int32_t seq_len,
                   int32_t kv_dim, int32_t kv_mul, int32_t head_size, const tensor::Tensor& mha_out,
                   const tensor::Tensor& query_tensor, const tensor::Tensor& score_tensor,
                   const tensor::Tensor& key_cache_tensor, const tensor::Tensor& value_cache_tensor,
                   base::DeviceType device_type, CudaConfig* config) {
  UNUSED(device_type);
  int32_t layer_offset = layer_index * seq_len * kv_dim;
  float* query = const_cast<float*>(query_tensor.ptr<float>());
  float* output = const_cast<float*>(mha_out.ptr<float>());

  float* key_cache = const_cast<float*>(key_cache_tensor.ptr<float>());
  float* value_cache = const_cast<float*>(value_cache_tensor.ptr<float>());

  cudaStream_t stream = config->stream;
  multi_head_attention_kernel<<<head_num, thread_num, 2 * head_size * sizeof(float), stream>>>(
      pos, seq_len, query, output, key_cache, value_cache, kv_dim, kv_mul, head_num,
      head_size, layer_offset);
}

}  // namespace kernel
