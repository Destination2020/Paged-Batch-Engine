...
class Qwen2Model : public Model {
 ...
 private:
  ...
  void attention_mha_paged(int32_t layer_idx, const tensor::Tensor& pos_tensor) const;
  ...
  void attention_qkv_paged(int32_t layer_idx, const tensor::Tensor& pos_tensor) const;
 ...
 private:
  ...
  bool use_paged_kv_ = true;
  mutable std::vector<std::unique_ptr<base::BlockAllocator>> block_allocators_;
  mutable std::unique_ptr<base::SequenceKVManager> seq_kv_manager_;
  tensor::Tensor block_table_gpu_;
};
}  // namespace model
